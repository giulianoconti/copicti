...
const Pagination = ({
  ...
  return (
    <div className={`pagination ${className}`} style={style}>
      ...
      {goPage ? <PageNumbers currentPage={currentPage} goPage={goPage} /> : <span className={`pagination number current`}>{currentPage}</span>}
      ...
      {goLastPage && (
        <button className="pagination-last-page" onClick={goLastPage} disabled={disabled || true || isLastPage}>
          &gt;&gt;
        </button>
      )}
    </div>
...
const PageNumbers = ({ currentPage, goPage }: { currentPage?: number; goPage?: (pageNumber: number) => void }) => {
